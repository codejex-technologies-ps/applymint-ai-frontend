...
export const profiles = pgTable("profiles", {
  ...
  availabilityStatus: text("availability_status").$type<
    typeof availabilityStatusEnum[number]
  >().default("available"),
  preferredWorkType: text("preferred_work_type").$type<
    typeof preferredWorkTypeEnum[number]
  >().default("full_time"),
  profileVisibility: text("profile_visibility").$type<
    typeof profileVisibilityEnum[number]
  >().default("public"),
...
export const insertProfileSchema = createInsertSchema(profiles, {
  ...
  location: z.string().max(100, "Location must be less than 100 characters")
    .optional(),
  website: z.string().regex(urlRegex, "Invalid website URL").optional().or(
    z.literal(""),
  ),
  linkedinUrl: z.string().regex(linkedinRegex, "Invalid LinkedIn URL")
    .optional().or(z.literal("")),
  githubUrl: z.string().regex(githubRegex, "Invalid GitHub URL").optional().or(
    z.literal(""),
  ),
  twitterUrl: z.string().regex(twitterRegex, "Invalid Twitter URL").optional()
    .or(z.literal("")),
  portfolioUrl: z.string().regex(urlRegex, "Invalid portfolio URL").optional()
    .or(z.literal("")),
  currentPosition: z.string().max(
    100,
    "Position must be less than 100 characters",
  ).optional(),
  company: z.string().max(100, "Company name must be less than 100 characters")
    .optional(),
  yearsOfExperience: z.number().int().min(
    0,
    "Years of experience cannot be negative",
  ).max(50, "Years of experience seems too high").optional(),
  ...
  credit: z.number().int().min(0, "Credits cannot be negative").default(5)
    .optional(),
});
